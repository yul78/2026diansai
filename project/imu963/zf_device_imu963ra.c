...
uint8 imu963ra_device_addr = IMU963RA_DEV_ADDR;
uint8 imu963ra_who_am_i = 0;
uint8 imu963ra_diag_scl_low = 0;
uint8 imu963ra_diag_scl_release = 0;
uint8 imu963ra_diag_sda_low = 0;
uint8 imu963ra_diag_sda_release = 0;
uint8 imu963ra_diag_ack_6b = 0;
uint8 imu963ra_diag_ack_6a = 0;
uint8 imu963ra_diag_who_6b = 0;
uint8 imu963ra_diag_who_6a = 0;
...
uint8 imu963ra_init (void)
{
...
#if IMU963RA_USE_SOFT_IIC
    soft_iic_init(&imu963ra_iic_struct, IMU963RA_DEV_ADDR, IMU963RA_SOFT_IIC_DELAY * 2U, IMU963RA_SCL_PIN, IMU963RA_SDA_PIN);

    gpio_low(IMU963RA_SCL_PIN);
    system_delay_ms(1);
    imu963ra_diag_scl_low = gpio_get_level(IMU963RA_SCL_PIN);
    gpio_high(IMU963RA_SCL_PIN);
    system_delay_ms(1);
    imu963ra_diag_scl_release = gpio_get_level(IMU963RA_SCL_PIN);

    gpio_low(IMU963RA_SDA_PIN);
    system_delay_ms(1);
    imu963ra_diag_sda_low = gpio_get_level(IMU963RA_SDA_PIN);
    gpio_high(IMU963RA_SDA_PIN);
    system_delay_ms(1);
    imu963ra_diag_sda_release = gpio_get_level(IMU963RA_SDA_PIN);

    imu963ra_iic_struct.addr = 0x6B;
    soft_iic_start(&imu963ra_iic_struct);
    imu963ra_diag_ack_6b = soft_iic_send_data(&imu963ra_iic_struct, 0x6B << 1);
    soft_iic_stop(&imu963ra_iic_struct);
    imu963ra_diag_who_6b = imu963ra_read_acc_gyro_register(IMU963RA_WHO_AM_I);

    imu963ra_iic_struct.addr = 0x6A;
    soft_iic_start(&imu963ra_iic_struct);
    imu963ra_diag_ack_6a = soft_iic_send_data(&imu963ra_iic_struct, 0x6A << 1);
    soft_iic_stop(&imu963ra_iic_struct);
    imu963ra_diag_who_6a = imu963ra_read_acc_gyro_register(IMU963RA_WHO_AM_I);

    if(0x6B == imu963ra_diag_who_6b)
    {
        imu963ra_iic_struct.addr = 0x6B;
        imu963ra_who_am_i = imu963ra_diag_who_6b;
    }
    else if(0x6B == imu963ra_diag_who_6a)
    {
        imu963ra_iic_struct.addr = 0x6A;
        imu963ra_who_am_i = imu963ra_diag_who_6a;
    }
    else
    {
        imu963ra_iic_struct.addr = 0x6A;
    ...
    imu963ra_device_addr = imu963ra_iic_struct.addr;
    if(0x6B != imu963ra_who_am_i)
    {
        return IMU963RA_INIT_MAIN_ERROR;
    }
#else
    ...
    do
    {
        ...
        if(imu963ra_acc_gyro_self_check())
        {
            ...
            return_state = IMU963RA_INIT_MAIN_ERROR;
            break;
        }
        ...
        switch(IMU963RA_ACC_SAMPLE_DEFAULT)
        {
            default:
            {
                ...
                return_state = IMU963RA_INIT_ACC_CFG_ERROR;
            }break;
        ...
        if(IMU963RA_INIT_OK != return_state)
        {
            break;
        }
        ...
        switch(IMU963RA_GYRO_SAMPLE_DEFAULT)
        {
            default:
            {
                ...
                return_state = IMU963RA_INIT_GYRO_CFG_ERROR;
            }break;
        ...
        if(imu963ra_mag_self_check())
        {
            ...
            return_state = IMU963RA_INIT_MAG_ERROR;
            break;
        }
        ...
        switch(IMU963RA_MAG_SAMPLE_DEFAULT)
        {
            default:
            {
                ...
                return_state = IMU963RA_INIT_MAG_CFG_ERROR;
            }break;
